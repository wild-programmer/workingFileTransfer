...
interface IpAvatarProps extends IComponentProps {
  id: number,
  history?: any;
  ipTypeNumber: number;
}

interface IpAvatarState {
  currentIndex: number,
  tabs: any[],
  cityAreaNum: number,
  hotCurrent: number,
...
@inject('detail')
@observer
export default class IpAvatar extends React.Component<IpAvatarProps, IpAvatarState> {
  constructor(props) {
    super(props);
            ...
            <div className="tab-content" style={{ "display": isShow1 }}>
              ...
              <div className="module-box">
                ...
                <div className="area-content blog-content">
                  ...
                  {
                    blogCurrent === 14 &&
                    <EchartLine container="echart-line2" xHot={xfansBlog} yHot={yfansBlog} subtext=""/>
                  }
                  {
                    blogCurrent !== 14 &&
              ...
              ipArtDetailList && ipArtDetailList.map((item, index) => {
                return (
                  <div className="tab-content flex-column " style={{ "display": isShow2 }} key={index}>
                    ...
                    {item.cooperationCase !== "" &&
                    <div className="module-box">
                      <p className="area-title"><img src={ic_cooperate} alt=""/>衍生品/合作案例</p>
                      <div className="area-content">
                        <div className="area-box flex-row justify-content-around">
                          {item.cooperationCase && JSON.parse(item.cooperationCase).map((v, k) => {
                            return (
                              <div className="box" key={k}>
                                <img src={`${item.picPrefix}${v.pic}`} alt=""/>
                                <p className="word-ellipsis">{v.title} </p>
