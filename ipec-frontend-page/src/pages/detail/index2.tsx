...
import { IpArt, IpStar } from "@pages/detail/components";
import Loading from '@components/loading';
import producer_img from '@assets/images/default_img_product.png';
...
export default class Detail extends React.Component<IProps, IDetailState> {
  ...
  async componentDidMount() {
    ...
    const { detail, nav_store } = this.props;
    ...
    let k = numberKV2[ipTypeNumber];
    this.setState({
      ipType: nameKeyValues[k]
    });
    console.log(this.state.ipType);
    let ipid = Number(params['id']);
    let user = sessionStorage.getItem("user");
    let dataParam = {}, dataParam2 = {}, dataParam3 = {}, dataParam4 = {};
    if (params.hasOwnProperty('id') && ipid > 0) {
      dataParam = { dayNumber: 10, ipid, typeId: 5 };
      dataParam2 = { dayNumber: 10, ipid, typeId: 10 };
      dataParam3 = { dayNumber: 10, ipid, typeId: 13 };
      dataParam4 = { dayNumber: 10, ipid, typeId: 14 };
      if (ipTypeNumber === 2) {
        if (user !== null) {
          const { userGuid } = JSON.parse(user);
          await detail.ipArtDetail({ userGuid, ipid });
        } else {
          await detail.ipArtDetail({ ipid });
        }
        ...
        await detail.getRelatedCase({ ipid });
        await detail.getProdctionData({ isUpcoming: 1, ipid, ipName, currentPage: 1, pageSize: 4 });
        await detail.getProdctionData({ isUpcoming: 0, ipid, ipName, currentPage: 1, pageSize: 2 });
  ...
  render() {
    const { nav_store, detail } = this.props;
