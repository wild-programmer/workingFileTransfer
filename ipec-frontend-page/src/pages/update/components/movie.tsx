...
export default class Avatar extends React.Component<IAvatarProps, IAvatarState> {
  ...
  async componentDidMount() {
    ...
    this.callback({ ipipIsShow: this.state.ipipIsShow });
    ...
    return (
      <div className="create-right-container flex-column">
        ...
        <div className="form-group form-540  flex-column">
          <label className="input-label">是否上映</label>
          <div className="radio-group">
            <div
              onClick={() => {
                const o = { ipipIsShow: 1 };
                this.setState(o);
                this.callback(o);
              }}
              className={`ip-radio flex-row align-items-center ${this.state.ipipIsShow === 1 ?
                "radio-selected" : ""}`}>
            ...
            <div
              onClick={() => {
                const o = { ipipIsShow: 2 };
                this.setState(o);
                this.callback(o);
              }}
              className={`ip-radio flex-row align-items-center ${this.state.ipipIsShow === 2 ?
                "radio-selected" : ""}`}>
              <div className="limit-custom-radio" />
          ...
          <div className="ip-release bankgroundGray">
            {this.state.ipipIsShow > 0 && <TimeInput
              callback={this.timeCallback}
              margin_left={this.state.ipipIsShow === 1 ? 3 : 9}
              placeholder="请选择上映时间"
              defaultValue="" />}
          </div>
        </div>
        ...
        <div className="form-group form-540  flex-column">
          ...
          <input
            onChange={e => this.callback({ companyGuidXf: e.target.value })}
            defaultValue={updateList.companyGuidXf}
            type="text" className="form-control" placeholder="如果有多个, 名字之间用英文逗号隔开" />
        </div> 
        <div className="form-group form-540 flex-column">
